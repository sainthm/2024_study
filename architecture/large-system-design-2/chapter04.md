...
- 메시지 큐 브로커의 상태 저장소에는 아래와 같은 정보가 저장
    - 소비자에 대한 파티션의 배치 관계
    - 각 소비자 그룹이 각 파티션에서 마지막으로 가져간 메시지의 오프셋
- 소비자 상태 정보 데이터가 이용되는 패턴:
    - 읽기와 쓰기가 빈번하게 발생하지만 양은 많지 않음
    - 데이터의 갱신은 빈번하게 일어나지만 삭제되는 일은 거의 없음
    - 읽기와 쓰기 연산은 무작위적 패턴을 보임
    - 데이터의 일관성이 중요
- 이러한 상태 데이터를 보관하는 데는 어떤 저장소 기술을 사용하는 것이 바람직할까?
    - 데이터의 일관성 및 높은 읽기/쓰기 속도에 대한 요구사항을 고려하였을 때, `주키퍼`와 같은 키-밸류 저장소를 사용하는 것이 바람직해보임
    - 한편 `카파카`는 오프셋 저장소로 주키퍼를 사용하다가 카프카 브로커로 이전

### 메타데이터 저장소

> 메타데티너 저장소에는 토픽 설정이나 속성 정보를 보관
> 
- 파티션 수, 메시지 보관 기간, 사본 배치 정보 등이 이에 해당
- 메타데이터는 자주 변경되지 않으며 양도 적음
- 하지만 높은 일관성을 요구하므로 이러한 데이터 보관에는 주키퍼가 적절함

...
- 새 서버로 교체할 때 운영 문제가 발생할 가능성은 클러스터 크기를 조정할 때보다 훨씬 낮음
    - 채널의 대규모 이동 사태를 초래하지 않기 때문에!
- 일반적인 장애 발생 프로세스와 비슷하게 진행:
    - 장애 발생 → 모니터링 소프트웨어에서 alert 발송 → 담당자는 서비스 디스커버리 컴포넌트의 해시 링 키에 매달린 값 갱신 → 장애 발생 노드를 대기중인 노드와 교체 → 교체 사실이 모든 웹소켓 서버에 전파 → 각 웹소켓 서버는 실행 중인 연결 핸들러에게 새 펍/섭 서버의 채널을 다시 구독하도록 알림 → 각 연결 핸들러는 구독 중인 채널의 목록을 유지 → 통지를 받으면 모든 채널을 해시 링과 대조하여 새 서버로 구독 관계를 다시설정해야하는지 검토

### 친구 추가/삭제

- 친구 추가 작업:
    - 해당 클라이언트에 연결된 웹소켓 서버의 연결 핸들러에게 알림
    - 새 친구의 펍/섭 채널을 구독
    - 새 친구 추가 시, 콜백 호출
    - 새 친구의 펍/섭 채널을 구독하라는 메시지 발송
    - 해당 메시지를 처리한 웹소켓 서버는 해당 친구가 활성화 상태인 경우, 가장 최근 위치 및 시각 정보를 응답 메시지에 담아 발송
- 친구 삭제 작업:
    - 친구 삭제 콜백 호출
    - 해당 친구의 펍/섭 채널 구독 취소 메시지를 웹소켓 서버에 발송


### 친구가 많은 사용자

- 친구가 많은 사용자가 시스템 성능 문제를 발생시킬 수 있을까?
- 친구는 양방향으로 가정
- 수천 명의 친구를 구독하는 데 필요한 펍/섭 구독 관계는 클러스터 내의 많은 웹소켓 서버에 분산
- 핫스팟 문제는 발생 X
- 이러한 친구들의 채널들이 모든 펍/섭 서버에 분산 → 결국 크게 부담을 주지 않음

### 주변의 임의 사용자

- 위치 정보 공유에 동의한 주변 사용자를 랜덤으로 보여줄 수 있게하려면 어떻게 해야할까?
- 지오해시에 따라 구축된 펍/섭 채널 풀을 두는 것
    - 격자마다 채널을 하나씩 마련
    - 격자 내의 모든 사용자는 해당 격자에 할당된 채널을 구독
    - 해당 지오해시 뿐아니라 주변 지오해시 격자를 담당하는 채널도 구독하면 격자 경계 부근에 있는 사용자를 효율적으로 처리 가능


### 레디스 펍/섭 외의 대안

- 라우팅 계층으로 채택한 레디스 펍/섭 대안이 있을까?
- 얼랭(Erlang)이 유용한 해결책이 될 수 있음
    - 사용자가 많지 않은 언어라 좋은 프로그래머를 구하기 어려울 수 있음
    - 팀내에 전문가가 있다면 좋은 대안이 될 수 있음
- 얼랭은 고도로 분산된 병렬 애플리케이션을 위해 고안된 프로그래밍 언어이자 런타임 환경
- 얼랭 에코시스템:
    - 언어 컴포넌트 뿐만 아니라 (얼랭 또는 엘릭서(Elixir))
    - 실행 환경 및 라이브러리도 있음 (얼랭 가상 머신 BEAM, 얼랭 런타임 라이브러리 OTP)
- 얼랭의 강점:
    - 경량 프로세스:
        - 얼랭 프로세스는 BEAM VM에서 실행되는 개체(entity)
        - 리눅스 프로세스 생성에 비해 엄청나게 저렴
        - 가장 작은 얼랭 프로세스는 300바이트 메모리 사용
        - 아무 일도 하지 않는 얼랭 프로세스는 CPU 자원 소모 X
    - 분산:
    - 운영 부담 낮음
    - 이슈 실시간 디버깅 지원
    - 배포 도구도 강력함
- 대체 방안:
    - 웹소켓 서비스 → 얼랭으로 구현
    - 레디스 펍/섭 클러스터 → 얼랭 애플리케이션으로 대체
    - 각 사용자 → 얼랭 프로세스로 표현

## 4단계: 마무리

- 주변 친구 기능의 설계안
    - 특정 사용자의 위치 정보 변경 내역을 친구에게 효율적으로 전달하는 시스템
- 핵심 컴포넌트:
...
## 2장 요약
![주변 친구](https://github.com/sainthm/2024_study/assets/54525036/d9369138-515d-4307-8c11-365b5080115b)

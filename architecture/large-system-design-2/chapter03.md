...
- `WebGL?`
    - WebGL은 웹 브라우저에서 3D 그래픽을 렌더링하기 위한 웹 그래픽스 라이브러리
    - WebGL은 JavaScript API로 제공되며, 이를 사용하여 웹 페이지에 3D 그래픽을 만들고 제어 가능
    - 기본적으로 WebGL은 OpenGL ES(Open Graphics Library for Embedded Systems)의 웹 버전으로 볼 수 있음
        - 모바일 장치나 다양한 플랫폼에서 3D 그래픽을 렌더링하기 위한 표준적인 그래픽 라이브러리
    - WebGL을 사용하면 웹 브라우저 상에서 다양한 3D 그래픽 응용 프로그램을 만들 수 있음
        - ex. 웹 게임, 시뮬레이션, 데이터 시각화, 교육용 응용 프로그램 등
    - WebGL은 하드웨어 가속을 사용하여 성능을 최적화하므로, 복잡한 3D 그래픽을 부드럽게 렌더링 가능
- 네트워크를 통해 이미지를 전송하는 대신 경로와 다각형 등의 벡터 정보를 전송
- 클라이언트는 수신된 경로와 다각형 정보를 통해 지도를 그려냄
- 벡터 타일의 장점:
    - 이미지에 비해 월등한 압축률
        - 이로 인해 네트워크 대역폭을 많이 아낄 수 있음
    - 매끄러인 지도 확대 경험
...
> 
- 최단 경로를 반환할 때, 도로 상황은 고려 X
- 도로 구조에만 의존하여 계산 수행
- 도로망 그래프는 거의 정적이므로 캐시 작업 용이
- `A* 경로 탐색 알고리즘`의 한 형태를 객체 저장소에 저장된 경로 안내 타일을 넣고 실행하는 로직:
    - 입력으로 출발지와 목적지의 위도/경도를 받음
    - 해당 위치 정보를 지오해시로 변환한 다음 출발지와 목적지 경로 안내 타일을 얻음
    - 출발지 타일에서 시작하여 그래프 자료 구조를 탐색
    - 탐색 범위를 넓히는 과정에서 필요한 주변 타일은 객체 저장소(혹은 캐시)에서 가져옴
    - 같은 지역의 다른 확대 수준 타일로도 연결이 존재함을 유의 (ex. To highway)
    - 최단 경로가 확보될 때까지 알고리즘은 검색 범위를 계속 확대해 나가면서 필요한 만큼 타일을 가져오는 작업을 반복

### 예상 도착 시간 서비스

> 기계 학습을 활용해 현재 교통 상황 및 과거 이력에 근거하여 예상 도착 시간을 계산
> 
- 실시간 교통 상황 데이터 + 앞으로 10~20분 뒤의 교통 상황을 예측 → 까다로움
- `알고리즘 차원`에서 풀어야함

### 순위 결정 서비스

> 필터링이 끝나고 남은 경로를 소요 시간 순으로 정렬하여 최단 시간 경로 k 개를 구한 다음 경로 안내 서비스에 결과 반환
> 
- 경로 계획 서비스는 ETA 예상치를 구하고 나면 순위 결정 서비스(ranker)에 관련 정보를 모두 전달
- 사용자가 정의한 필터링 조건 적용
    - ex. 유료 도로 제외, 고속도로 제외 등

### 중요 정보 갱신 서비스들

> 해당 부류의 서비스는 카프카 위치 데이터 스트림을 구독하고 있다가 중요 데이터를 비동기적으로 업데이트하여 그 상태를 항상 최신으로 유지하는 역할 담당
> 
- 실시간 교통 정보 데이터베이스
- 경로 안내 타일 등
- 경로 안내 타일 처리 서비스:
    - 도로 데이터에 새로 발견된 도로, 폐쇄되었음이 확인된 도로 정보를 반영하여 경로 안내 타일을 지속적으로 갱신
    - 최단 경로 서비스는 더 정확한 결과를 낼 수 있음
- 실시간 교툥 상황 서비스:
    - 활성화 상태 사용자가 보내는 위치 데이터 스트림에서 교통 상황 정보를 추출
    - 결과값 정보는 실시간 교통 상황 데이터베이스에 반영
    - 예상 도착 시간 서비스가 더욱 정확한 결과를 내는 데 사용

### 적응형 ETA와 경로 변경

- 현재까지의 설계안에서 적응형 ETA와 경로 변경은 허용 X
- 해당 문제를 건드리려면 서버는 현재 경로 안내를 받고 있는 모든 사용자를 추적하면서 교통 상황이 달라질 때마다 각 사용자의 ETA를 변경해주어야함
- 현재 경로 안내를 받고 있는 사용자는 어떻게 추적할까?
- 수백만 경로 가운데 교통 상황 변화에 영향을 받는 경로와 사용자를 효율적으로 가려낼 방법은 무엇일까?
- 간단하지만 효율적이지 않은 방법: (a.k.a 전수조사)
    - 테이블에 보관된 레코드 수 → n
    - 안내되는 경로의 평균 길이 → m
    - 교통 상황 변화에 영향 받는 모든 사용자 검색의 시간 복잡도 `O = n * m`
- 다른 접근법:
    - 경로 안내를 받는 사용자 각각의 현재 경로 안내 타일, 그 타일을 포함하는 상위 타일, 그 상위 타일의 상위 타일을 출발지와 목적지가 모두 포함된 타일을 찾을 때까지 재귀적으로 더하여 보관
    - 어떤 타일의 교통 상황이 변경되었을 때, 경로 안내 ETA가 달라지는 사용자는 해당 사용자의 데이터베이스 레코드 마지막 타일에 그 타일이 속하는 사용자
    - 이외의 사용자에게는 아무 영향 X
    - 검색 시간 복잡도 `O = n` → 위의 방법보다는 조금 더 효율적
    - 하지만 교통 상황이 개선되었을 때 해야 하는 일까지 해결해주지는 않음
    - 현재 경로 안내를 받는 사용자가 이용 가능한 경로의 ETA를 주기적으로 재계산하여 더 짧은 ETA를 갖는 경로가 발견되는 것을 알리는 방식으로 해결 가능

### 전송 프로토콜

- 경로 안내 중에 경로의 상황이 변경될 수 있으므로 데이터를 모바일 클라이언트에 전송할 안정적인 방법이 필요
- 서버에서 클라이언트로 데이터를 보내는데 활용할 수 있는 프로토콜:
    - 모바일 푸시 알림:
        - 보낼 수 있는 메시지 크기가 매우 제한적 (iOS → 최대 4096 바이트)
        - 웹 애플리케이션 지원 X
        - 본 설계안 사용 X
    - 롱 폴링
        - 본 설계안 사용 X
    - 웹소켓:
        - 서버에 주는 부담이 크지 않아서 일반적으로 롱 폴링보다 좋은 방법
        - 양방향 통신 지원
            - 패키지나 상품이 목적지에 가까워졌을 때는 실시간 양방향 통신이 필요한 경우도 있기 때문
        - 본 설계안에서 사용
    - 서버 전송 이벤트(SSE)
    - 등등 존재
...
> 핵심 기능을 지원하는 단순화된 구글 맵 애플리케이션 설계
> 
- 위치 갱신
- ETA
- 경로 계획
...
- 기업 고객 대상으로 `중간 경유지 설정 기능`
    - 적용 시 도어대시, 우버, 리프트(Lyft) 같은 배달 서비스에 유용할 것!
